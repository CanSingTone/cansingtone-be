...
### 음역대 기반 추천
사전에 데이터베이스에 있는 모든 곡들에 대해 음역대를 분석 및 저장합니다. 사용자는 음역대 기반 추천을 받기 위해 음역대 테스트를 진행, 자신이 낼 수 있는 최고음과 최저음을 앱 내에서 녹음한 후 녹음 파일을 서버로 전송합니다. 서버로 전송된 녹음 파일은 MIDI로 변환됩니다. MIDI는 전자 악기끼리 디지털 신호를 주고받기 위해 각 신호를 규칙화한 일종의 규약입니다. 녹음 파일에 저장된 음성 데이터는 MIDI로 변환되면서 음계로 표현되고, 최저음과 최고음을 확인하기에 편리한 형태로 가공됩니다. 사용자는 분석된 음역대에 맞는 곡들을 추천받습니다.
...
AI 모델은 사람의 고유한 특징이라고 볼 수 있는 음색을 분석하기 위해 가수 분류 모델을 활용하였습니다. 해당 모델의 입출력 형태를 수정하여 음색 유사도를 확인하는데 사용할 수 있도록 하였습니다. 우선 MR 분리를 통해 목소리만 입력으로 사용될 수 있도록 했고, 가창 등장 부분 분리를 통해 노이즈를 최소화하였습니다. 또한 이는 모든 시간에 목소리가 있기에 5초 단위로 slice를 하는 것을 가능케 했고, 5초 단위의 slice를 하나의 입력으로 사용함으로써 ‘노래’의 특징이 아닌 ‘목소리(음색)'의 특징에 집중할 수 있도록 했습니다. 이러한 입력 데이터로 모델을 학습시켜 음색에 따른 분류를 수행할 수 있도록 했습니다.
